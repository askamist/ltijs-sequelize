...
class Database {
  ...
  #databaseCleanup = async () => {
    ...
    let res
    res = await this.#Models.idtoken.destroy({
      where: {
        createdAt: {
          [Sequelize.Op.lte]: Date.now() - this.#ExpireTime.idtoken
        }
      }
    })
    provDatabaseDebug('Expired idtoken: ' + res)

    res = await this.#Models.contexttoken.destroy({
      where: {
        createdAt: {
          [Sequelize.Op.lte]: Date.now() - this.#ExpireTime.contexttoken
        }
      }
    })
    provDatabaseDebug('Expired contexttoken: ' + res)

    res = await this.#Models.accesstoken.destroy({
      where: {
        createdAt: {
          [Sequelize.Op.lte]: Date.now() - this.#ExpireTime.accesstoken
        }
      }
    })
    provDatabaseDebug('Expired accesstoken: ' + res)

    res = await this.#Models.nonce.destroy({
      where: {
        createdAt: { [Sequelize.Op.lte]: Date.now() - this.#ExpireTime.nonce }
      }
    })
    provDatabaseDebug('Expired nonce: ' + res)

    res = await this.#Models.state.destroy({
      where: {
        createdAt: { [Sequelize.Op.lte]: Date.now() - this.#ExpireTime.state }
      }
    })
  ...
  constructor (database, user, pass, options) {
    this.#sequelize =
      pass !== undefined
        ? new Sequelize(database, user, pass, options)
        : new Sequelize(database, user)
    this.#dialect = this.#sequelize.getDialect()
    this.#Models = {
      idtoken: this.#sequelize.define(
        'idtoken',
        {
          iss: {
            type: Sequelize.TEXT
      ...
      contexttoken: this.#sequelize.define(
        'contexttoken',
        {
          contextId: {
            type: Sequelize.TEXT
      ...
      platform: this.#sequelize.define(
        'platform',
        {
          platformName: {
            type: Sequelize.TEXT
      ...
      platformStatus: this.#sequelize.define(
        'platformStatus',
        {
          id: {
      ...
      publickey: this.#sequelize.define(
        'publickey',
        {
          kid: {
            type: Sequelize.STRING,
      ...
      privatekey: this.#sequelize.define(
        'privatekey',
        {
          kid: {
            type: Sequelize.STRING,
      ...
      accesstoken: this.#sequelize.define(
        'accesstoken',
        {
          platformUrl: {
            type: Sequelize.TEXT
  ...
  async Get (ENCRYPTIONKEY, table, info) {
    ...
    for (let item of queryResult) {
      ...
      if (ENCRYPTIONKEY) {
        const temp = item
        item = JSON.parse(
          await this.Decrypt(item.data, item.iv, ENCRYPTIONKEY)
        )
        if (temp.createdAt) {
  ...
  async Modify (ENCRYPTIONKEY, table, info, modification) {
    ...
    if (ENCRYPTIONKEY) {
      let result = await this.#Models[table].findOne({
        where: info,
        raw: true
      })
      if (result) {
